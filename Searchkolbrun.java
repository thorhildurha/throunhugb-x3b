      ...
      @Override
        public void actionPerformed(ActionEvent e)
        {
            ...
            if(priceText.getText().trim().isEmpty()) {
                ...
                Book[] books = mock.searchByUser(new Owner());
                for (int i = 0; i < books.length; i++)
                {
                    Book book  = books[i];
                    if(titleText.getText().trim().equalsIgnoreCase(book.getName()) || authorText.getText().trim().equalsIgnoreCase(book.getAuthor())
                    || isbnText.getText().trim().equalsIgnoreCase(book.getIsbn())) {
